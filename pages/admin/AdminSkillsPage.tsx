                ...
                <div className="md:col-span-1">
                    <Card>
                        <CardHeader className="flex justify-between items-center">
                            ...
                            <Button size="sm" onClick={() => setIsCreatingSkill(!isCreatingSkill)}>{isCreatingSkill ? 'Cancel' : '+'}</Button>
                        </CardHeader>
                        <CardContent>
                            {isCreatingSkill && (
                            ...
                            {isLoadingSkills ? <Spinner/> : (
                                <ul className="space-y-2">
                                    {skills.map(skill => (
                                        <li key={skill.id}>
                                            <button onClick={() => handleSelectSkill(skill)} className={`w-full text-left p-2 rounded-md transition-colors ${selectedSkill?.id === skill.id ? 'bg-blue-600 text-white' : 'hover:bg-slate-100'}`}>
                                                {skill.name}
                                            </button>
                                        </li>
                                    ))}
                    ...
                    {selectedSkill ? (
                        <Card>
                            <CardHeader className="flex justify-between items-center">
                                ...
                                <Button onClick={() => setIsCreatingQuestion(true)} disabled={isCreatingQuestion}>Add Question</Button>
                            </CardHeader>
                            <CardContent>
                                {isCreatingQuestion && (
                                ...
                                {isLoadingQuestions ? <Spinner /> : (
                                    <div className="space-y-4 mt-6">
                                        {questions.length > 0 ? questions.map((q, index) => (
                                            ...
                                            !isCreatingQuestion && <p>No questions found for this skill.</p>
                                        )}
                                    </div>
                                )}
                            </CardContent>
